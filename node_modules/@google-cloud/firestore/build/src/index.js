...
class Firestore {
        ...
        if (url !== null) {
            if ((settings.servicePath !== undefined &&
                ...
                console.warn(`The provided host (${url.hostname}) in "settings" does not ` +
                    `match the existing host (${settings.servicePath ?? settings.apiEndpoint}). Using the provided host.`);
            }
            settings.servicePath = url.hostname;
    ...
    async initializeIfNeeded(requestTag) {
        ...
        if (this._projectId === undefined) {
            try {
                ...
                // need to record it.
                this._traceUtil.recordProjectIdAndResourceName(this.projectId, this.databaseId);
            }
            catch (err) {
    ...
    createCallOptions(methodName, retryCodes) {
        ...
        if (retryCodes) {
            ...
            callOptions.retry = new (require('google-gax/fallback').RetryOptions)(retryCodes, retryParams);
        }
        return callOptions;
    }
    /**
